...
export default function BlendBuilder() {
  ...
  const flavorProfile = {
    aroma: Math.round(currentBlend.reduce((acc, c) => acc + (c.aroma * (c.percentage / 100)), 0)),
    body: Math.round(currentBlend.reduce((acc, c) => acc + (c.body * (c.percentage / 100)), 0)),
    acidity: Math.round(currentBlend.reduce((acc, c) => acc + (c.acidity * (c.percentage / 100)), 0)),
    sweetness: Math.round(currentBlend.reduce((acc, c) => acc + (c.sweetness * (c.percentage / 100)), 0)),
    aftertaste: Math.round(currentBlend.reduce((acc, c) => acc + (c.aftertaste * (c.percentage / 100)), 0)),
  };
  ...
  if (currentBlend.length === 0) {
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground p-8 text-center">
            ...
            <Link href="/portfolio" className="btn-paris">Return to Portfolio</Link>
        </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-8 md:p-16 flex flex-col md:flex-row gap-16 font-sans">
      ...
      <div className="w-full md:w-1/2 space-y-12">
        ...
        <div className="space-y-8">
          {currentBlend.map((item) => (
            <div key={item.id} className="card-paris bg-card-bg p-6 group">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="relative w-20">
                    <input 
                        ...
                        value={item.percentage}
                        onChange={(e) => updatePercentage(item.id, parseInt(e.target.value))}
                        className="w-full bg-background border border-border-color rounded-sm px-2 py-1 text-right focus:border-accent-gold focus:outline-none font-mono"
                    />
        ...
        {/* Total & Validation */}
        <div className={`p-6 border ${totalPercentage === 100 ? 'border-accent-gold bg-accent-gold/5' : 'border-red-500/30 bg-red-500/5'} flex justify-between items-center transition-colors`}>
        ...
        <div className="bg-black text-white p-8 relative overflow-hidden">
            <h3 className="font-serif text-accent-gold font-bold mb-2 flex items-center gap-2">
                The Legend Says...
            </h3>
      ...
      <div className="w-full md:w-1/2 flex flex-col items-center">
         ...
         {/* Vis Mode Toggle */}
         <div className="flex border border-border-color mb-12">
            {['petal', 'bar', '3d'].map(m => (
                <button 
                    key={m}
                    onClick={() => setVisMode(m as any)}
                    className={`px-6 py-2 text-xs uppercase tracking-widest transition-all ${visMode === m ? 'bg-foreground text-background' : 'hover:bg-black/5'}`}
                >
                    {m === '3d' ? 'Heatmap' : m}
                </button>
            ))}
         ...
         <div className="relative w-full aspect-square max-w-md bg-card-bg border border-border-color flex items-center justify-center mb-12 p-8">
             {visMode === 'petal' && (
                 ...
                 <div className="w-full h-full flex flex-col justify-center gap-4">
                     {currentBlend.map(b => (
                         <div key={b.id}>
                             ...
                             <div className="h-2 bg-border-color overflow-hidden">
                                 <div className="h-full bg-foreground" style={{ width: `${b.percentage}%` }} />
                             </div>
                         </div>
                     ))}
                 </div>
             ...
             {visMode === '3d' && (
                 <div className="text-center opacity-40">
                     ...
                     <div className="w-32 h-32 mx-auto bg-gradient-to-tr from-foreground to-accent-gold opacity-20 rounded-full animate-pulse blur-xl" />
                 </div>
             )}
         </div>
         ...
         <div className="text-center w-full max-w-md">
             <div className="flex justify-between items-end border-b border-border-color pb-4 mb-8">
                ...
                <span className="font-mono text-4xl font-bold">€{totalPrice}</span>
             </div>
