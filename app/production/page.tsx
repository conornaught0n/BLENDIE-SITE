  ...
  return (
    <div className="h-full flex flex-col justify-center max-w-5xl mx-auto">
      <div className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4 text-white">Roastery OS</h1>
        <p className="text-lg text-white/60">Select your workspace module.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {modules.map((mod) => (
          <Link 
            key={mod.href} 
            href={mod.href}
            className={`group p-8 rounded-2xl border transition-all hover:scale-[1.02] ${mod.color} border-opacity-50 hover:bg-opacity-30`}
          >
            <div className="flex items-start justify-between mb-4">
              <span className="text-4xl">{mod.icon}</span>
              <span className="opacity-0 group-hover:opacity-100 transition-opacity text-sm font-bold uppercase tracking-widest">Enter →</span>
            </div>
